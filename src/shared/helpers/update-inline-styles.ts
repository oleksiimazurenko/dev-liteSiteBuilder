...
const updateInlineStyles = async (
	...
	const serializeInlineStyles = (
		...
		for (let i = 0; i < style.length; i++) {
			...
			inlineStyles[camelCasePropertyName] = style.getPropertyValue(propertyName)
		}
	...
	// Фцнкция для получения ключа с помощью которого мы будем обновлять стили в базе данных
	const getKey = (style: string): string => {
		...
		if (hasDataAttribute) {
			...
			if (dataAttribute === 'data-section') {
				const a = ruleSectionStyles.find(item => Object.keys(item)[0] === style)
